...
def get_gene_name(hmmsearch_output):
    ...
    for hmmsearch_file in hmmsearch_files:
        with gzip.open(hmmsearch_path + hmmsearch_file, 'rt') as hmmsearch_output:
            ...
            if species in species_list:
                # Get gene name
                gene_name = get_gene_name(hmmsearch_output)

                # Retrieve location info of gene
                for gff_file in gff_files:
                    if species in gff_file:
                        with gzip.open(gff_path + gff_file, 'rt') as gff_output:
                            fasta_header = get_location(gff_output, gene_name)[0]
                            orientation = get_location(gff_output, gene_name)[1]
